...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the reason why the ${ticker} stock price shifted on ${date}. Give what event happened that day. If live data is shown for a specific date, put Could Not Find for that dates line. Be in present tense and say the future impact the event will have on the companies stock. Everything for a specific day must be on one continuous line.`;
