...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the reason (a real world event, company announcement/release, etc) why the ${ticker} stock price significantly shifted following ${date}. Formatting: Be in present tense and ensure the data is on one continuous line. Put Not Found if approriate data is not found or`;
