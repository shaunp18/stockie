...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the real world event or ${ticker} company announcement/release that happened on/near ${date}. Formatting: Ensure the data for a specific date is on one continuous line. Put no significant event occured if approriate data is not accessible.;`;
