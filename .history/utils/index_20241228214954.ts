...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Provide detailed information on the real-world event or company-related announcement that influenced the significant spike in the stock price of the company represented by the ticker symbol ${ticker} on or near the date ${date}. The event could include, but is not limited to, major company announcements (e.g., earnings reports, mergers/acquisitions, partnerships, new product launches), significant regulatory changes, industry-wide developments, or macroeconomic factors. \n Focus on events that plausibly correlate with or directly caused the observed stock price movement. If the specific event cannot be identified, include contextual data such as major news headlines, market sentiment, or trends for the company and its sector during that time. \n Formatting: \n Summarize the event(s) in a concise, single line.
If no significant event occurred or if data is inaccessible, state explicitly: 'No significant event occurred during this time.' \n Ensure accuracy and relevance to ${ticker} and the provided date.`;
