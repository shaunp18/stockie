...
export async function fetchStockNews(ticker: string, date: []) {
  ...
  const prompt = `Tell me the reason (a real world event or company announcement/release, etc) why the ${ticker} stock price significantly shifted on/near ${date}. If live data is NOT shown for a specific date, put Data Not Available for that dates line. Be in present tense and say the future impact the event will have on the companies stock. Everything for a specific day must be on one continuous line.`;
